 ...
 *    	where rigidigty (in s^(1/n)Pa) is the flow law paramter in the flow law
 *sigma=B*e(1/n) (Nye, p2000).
 */
...
IssmDouble GBSH2O(IssmDouble temperature, IssmDouble phi) {
  ...
  IssmDouble d =
      (13.4e-6 / pow(f, 0.5)) * (rd / 1.5e-6) * pow((0.05 / phi), 0.5);
  ...
  if (temperature >= 273.15)
    _printf0_("H2O ICE - GUARANTEED MELTING. Some temperature values are "
              "beyond 273.15K.\n");
